...
int main()
{
  ...
  // CHECK-DAG: {{^}}[[MASTER_ID]]: ompt_event_loop_begin: parallel_id=[[PARALLEL_ID]], parent_task_id={{[0-9]+}}, codeptr_ra=[[LOOP_BEGIN_RETURN_ADDRESS:0x[0-f]+]]{{[0-f][0-f]}}
  // CHECK-DAG: {{^}}[[MASTER_ID]]: fuzzy_address={{.*}}[[LOOP_BEGIN_RETURN_ADDRESS]]
  // CHECK-DAG: {{^}}[[MASTER_ID]]: fuzzy_address={{.*}}[[LOOP_BEGIN_RETURN_ADDRESS]]
  // CHECK-DAG: {{^}}[[MASTER_ID]]: fuzzy_address={{.*}}[[LOOP_BEGIN_RETURN_ADDRESS]]
  // CHECK-DAG: {{^}}[[MASTER_ID]]: fuzzy_address={{.*}}[[LOOP_BEGIN_RETURN_ADDRESS]]
  // CHECK-DAG: {{^}}[[MASTER_ID]]: ompt_event_loop_end: parallel_id=[[PARALLEL_ID]], task_id={{[0-9]+}}, codeptr_ra=[[LOOP_END_RETURN_ADDRESS:0x[0-f]+]]{{[0-f][0-f]}}
  // CHECK-DAG: {{^}}[[MASTER_ID]]: fuzzy_address={{.*}}[[LOOP_END_RETURN_ADDRESS]]
  // CHECK-DAG: {{^}}[[MASTER_ID]]: ompt_event_parallel_end: parallel_id=[[PARALLEL_ID]], task_id={{[0-9]+}}, invoker={{[0-9]+}}, codeptr_ra=[[PARALLEL_RETURN_ADDRESS]]
  // CHECK-DAG: {{^}}[[MASTER_ID]]: current_address=[[PARALLEL_RETURN_ADDRESS]]
