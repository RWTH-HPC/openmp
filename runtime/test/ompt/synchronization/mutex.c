...
int main()
{
  ...
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_init_nest_lock: wait_id=[[WAIT_ID:[0-9]+]], hint={{[0-9]+}}, impl={{[0-9]+}}, codeptr_ra={{0x[0-f]+}}
  ...
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_wait_critical: wait_id=[[WAIT_ID:[0-9]+]], hint={{[0-9]+}}, impl={{[0-9]+}}, codeptr_ra={{0x[0-f]+}}
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_acquired_critical: wait_id=[[WAIT_ID]], codeptr_ra={{0x[0-f]+}}
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_release_critical: wait_id=[[WAIT_ID]], codeptr_ra={{0x[0-f]+}} 

  // atomic cannot be tested because it is implemented with atomic hardware instructions
  // disabled_CHECK: {{^}}[[MASTER_ID]]: ompt_event_wait_atomic: wait_id=[[WAIT_ID:[0-9]+]], hint={{[0-9]+}}, impl={{[0-9]+}}, codeptr_ra={{0x[0-f]+}}
  // disabled_CHECK: {{^}}[[MASTER_ID]]: ompt_event_acquired_atomic: wait_id=[[WAIT_ID]], codeptr_ra={{0x[0-f]+}}

  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_wait_ordered: wait_id=[[WAIT_ID:[0-9]+]], hint={{[0-9]+}}, impl={{[0-9]+}}, codeptr_ra={{0x[0-f]+}}
