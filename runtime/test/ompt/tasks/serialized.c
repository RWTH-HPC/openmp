...
int main()
{
  ...
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_task_create: parent_task_id={{[0-9]+}}, parent_task_frame.exit=[[NULL]], parent_task_frame.reenter=(nil), new_task_id={{[0-9]+}}, parallel_function=0x{{[0-f]+}}, task_type=ompt_task_initial=1, has_dependences=no
  ...
  // CHECK: {{^}}[[MASTER_ID]]: level 0: parallel_id=[[PARALLEL_ID]], task_id=[[IMPLICIT_TASK_ID]], exit_frame=[[EXIT]], reen
