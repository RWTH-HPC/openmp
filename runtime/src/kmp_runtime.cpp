...
//            parent_task_info = &(this_thr->th.th_current_task->td_parent->ompt_task_info);
    ...
    if (ompt_enabled && this_thr->th.ompt_thread_info.state != omp_state_overhead) {
        ...
        __ompt_lw_taskteam_init(&lw_taskteam, this_thr, global_tid,
                NULL, &ompt_parallel_data, codeptr);
//        exit_runtime_p = &(lw_taskteam.ompt_task_info.frame.exit_runtime_frame);
        ...
        implicit_task_data = &(this_thr->th.th_current_task->ompt_task_info.task_data);
    ...
    )
{
    ...
    if (ompt_enabled) {
        __ompt_get_task_info_internal(0, NULL, &parent_task_data, &ompt_frame, NULL, NULL);
        ...
        if ( parent_team == master_th->th.th_serial_team ) {
            ...
            if (ompt_enabled) {
                __ompt_lw_taskteam_init(&lw_taskteam, master_th, gtid,
                    unwrapped_task, &ompt_parallel_data, return_address);
                exit_runtime_p = &(lw_taskteam.ompt_task_info.frame.exit_runtime_frame);
...
// TODO: revert workaround for Intel(R) 64 tracker #96
    ...
    KMP_CHECK_UPDATE_SYNC(team->t.ompt_team_info.master_return_address, return_address);
#endif
...
#if OMPT_SUPPORT
    if (! initial_thread && ompt_enabled) {
        ...
        if (ompt_callbacks.ompt_callback(ompt_callback_task_create)) {
            ompt_callbacks.ompt_callback(ompt_callback_task_create)(
                ...
                ompt_task_initial,
                0,
                NULL); // initial task has nothing to return to
        }
